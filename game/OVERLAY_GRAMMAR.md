...
| **Small solid ring**, cell-sized, centred on a ROAD CELL | a MARKED CELL — "this one, specifically" | `board.gd` (`mark_road_answer`, one `Line2D` ring per road cell) — the reserved channel's second tenant, lit for `Hud.message_seconds(Hud.ROLE_NOTICE)` seconds on every cell that IS road, the instant `sole_legal_plant_for()` finds no single packet to point at for a road-only plant refused on grass (plant-tower-defense-oxf1). The sole-cover road rings were the FIRST tenant and cycle 179 removed them on a player report ("these awful yellow circles ... please remove them") for being persistent and countable against the whole garden; this tenant is neither — it is a rare, timed flash over a FIXED set (the road, which does not grow as the player builds), in `GardenTheme.LEAF` rather than the removed cue's yellow. Same radius as the removed rings (9 px), because the report was about persistence and colour, not size |
