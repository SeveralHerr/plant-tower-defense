...
func _physics_process(delta: float) -> void:
	...
	if wall != null:
		...
		# `halts_to_eat()` is where the `return` went (plant-tower-defense-rn4p). It is
		# true for every whole-body pest, so this is the same stop it always was; the
		# Cutworm bites the wall on its way past instead, because a wall in front of its
		# head is not standing in front of the other fourteen cells of it.
		if halts_to_eat():
			return
	if eats_in_passing():
