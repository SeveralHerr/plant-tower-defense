...
- **The notebook's HINTS page has its own 300px ceiling, and the whole repo only knows
  ...
  three rows from `SHELF_ROW_TOP` 3, `HINT_ROW_PITCH` 98 and a 94px row, and
  `hint_pages_needed()` (`:402`) pages past that. Cycle 140 added a sixth hint and its card
  overflowed by 35px — caught by
  `test_the_notebook_hints_page_gives_back_a_hint_that_was_never_shown`, whose message even
  names the two fixes ("trim the card or grow HINT_NOTE_HEIGHT"). **The gap is that nothing
  says the two ceilings are the same number for different reasons**, so the next person to
  price a teaching surface will find the legend's argument and think it is the only one.
  Cheap fix: one sentence in `cue_legend.gd`'s audit block pointing at `hints_capacity()`,
  since that block is where anyone pricing a surface will already be reading.

- **Nine MORE citations were already dead, in three files this cycle did not touch for that
  reason.** Same class as cycle 139's eleven and the same discovery mechanism — an
  ...
  are six. Both are now written to derive their number from the tool that counts it. Twenty
  dead citations across two cycles is not an accident rate; it is the detection gap
  `plant-tower-defense-nalv` describes, and this is its second sighting.

- **Relocating citations by script broke one, exactly the way `-2174` says it will.** A
  regex replacing the START of a range moved the first number and left the second, producing
  an INVERTED range — caught only because `citation_check` plain mode reports an
  out-of-range target. (The actual numbers are left out on purpose. Writing them here as an
  example made them a live citation and failed the checker on the spot — which is
  `kanban-idea-pass` rule 1's newest paragraph, added last cycle by the same hand that then
  walked into it while writing this bullet. The rule is right and it is easy to break.)
  That is the strongest concrete
  argument yet for the relocator bead's own design premise: **refusing is the product.** A
  relocator that renumbers confidently produces citations that resolve, look checked, and
  point somewhere nobody chose — and the failure here was visible only because the arithmetic
  happened to go backwards. The same bug going forwards would have been silent.
...
- **The cue vocabulary now has five taught marks and the sixth hint is spent, so the next
  ...
  with no teaching at all — cannot be taught by adding a row, a hint, or a card; it needs one
  of the three budgets raised first. Worth deciding WHICH before the next cue arrives, rather
  than discovering it during the cycle that ships one, which is how the lane-pressure hatch
  ended up untaught.

### New in cycle 139 — the recoil landed on five callers, and eleven citations were dead where nobody was looking

- **The pest recoil inherited every caller of `flash_hit()`, and one of them never damaged
  anything.** `Pest._flinch_left` is armed in exactly one place, `game/pest.gd:1698` inside
  `flash_hit()` — grepped for the field rather than for the verb, because the field is what
...
- **Eleven `kanban.md` citations into `game/pest.gd` were wrong by roughly eight hundred
  lines, every checker was green over them for an unknown number of cycles, and the only
  thing that revealed them was an unrelated edit.** This is a hole in the checker, not in
  anyone's care. `citation_check.py` plain mode proves a line EXISTS and says so in its own
  `NOT COVERED:` output; `--snapshot`/`--against` compares the TEXT at a citation before and
  after this cycle's edits — so a citation that was **already** pointing at the wrong line
  ...
  offset restore looking perfect.
  What would actually close it: let a citation optionally name the symbol it means —
  `` `game/pest.gd:1527` (`_gait`) `` — and have `citation_check` verify the named symbol's
  `func`/`const`/`var` declaration is inside the cited range. That is a one-file change to a
  checker this repo owns, it needs no snapshot, and unlike `--against` it can fire on a
  citation that went stale three cycles ago in a file nobody has touched since. Filed as a
  bead; the pest.gd eleven are already repointed.

### New in cycle 138 — a player asked what the marks meant, and the repo already knew
...
- **The cue vocabulary has outgrown the one surface that teaches it, and the overflow is
  ...
  for. What it costs: `page_for_kind` returns the FIRST page of a kind
  (`game/notebook_screen.gd:299`) and `legend_note_text()` reads that one entry, so both
  need a per-page answer before a second page can say anything different from the first.

- **The grammar doc's own derivation recipe is blind to the two cues that were built to
  be checkable, and this is the SECOND time that has been written down.**
  `game/cue_legend.gd:61-74` records it in full — the doc's recipe is
  `grep -n "draw_arc(\|draw_circle(\|draw_line(\|draw_rect(" game/*.gd`, and
  `Board.mark_dead_ground` / `Board.mark_deferred_road` paint Line2D CHILDREN rather than
  calling any of those. Board's own header (`game/board.gd:811`) says why they had to: "a
  headless run paints no frame, so a `_draw()` here would be a cue no gate could ever
  see". So the census misses precisely the two cues that were made gateable, and the
  comment ends "reported to whoever owns OVERLAY_GRAMMAR.md rather than fixed here". That
  report has now sat unread through several cycles and cycle 138 rediscovered it from the
  player's side. This is `.claude/skills/derive-the-list/SKILL.md`'s failure in its purest
  form: a hand-run grep IS a list, and one that names an API rather than an effect goes
  stale the moment a cue changes renderer. A checker that walks `game/*.gd` for BOTH
  shapes — the four draw calls and `Line2D.new()` — and compares the count against the
...
- **A hint that fires from `_refresh` needs `Hud.row_is_quiet` and there is no gate that
  ...
  wrote the fifth hint without it anyway and shipped 11 refused messages into a realistic
  run before `test_a_realistic_run_refuses_no_messages_and_evicts_none` caught it. That
  test is the gate, and it is a good one — but it fires on the SYMPTOM three files away
  from the cause and its message ("A non-zero refusal here is a caller stacking copies
  into a full queue") does not name the caller. A checker that finds every
  `spend_hint(...)` call site reachable from `_refresh` and requires a `row_is_quiet()`
  guard above it would name the file. Two of the five hint sites are level-triggered now
  (`game/game.gd:1774` and `game/game.gd:1445`), so the pattern has a population.
...
- **The weather banner has never once been visible to a player, and no gate could see
  ...
  `game/game.gd:471`, which reaches `hud.show_weather(next)` at `game/game.gd:471`; ten
  lines later, in the same function and the same call stack, `game/game.gd:471` calls
...
- **Three roster roles are genuinely uncovered, and two that `-ibvb` lists are not.**
  ...
  (the only `.heal(` on a plant is `game/game.gd:452`, the rain weather effect), and husks
...
- **Four `show_message` sites are edge-triggered only by an explicit latch.**
  `docs/message_trigger_audit.md` classifies all 17; the four at `game.gd:463`, `:1349`,
  `:1725` and the packet-reveal loop are edge ONLY because `_wave_live`, `_flight_noted`,
...
- **More waves, and bosses.** The ask: real additional waves in the table, and at least
  ...
  **AUDITED (cycle 76): DRIFTED — two of the three factual claims are false.** The
  ...
  and the mechanic itself is documented at `game/game.gd:936` — a pest whose species names
  ...
  **What is genuinely unbuilt** is narrower than the bullet reads, and it is the half the
  ...
  `game/game.gd:946`. So "a mechanic that isn't just armoured but more" shipped, and
...
- **Nothing in the game teaches upgrading, and upgrading is the whole campaign.** Two full
  ...
  that only exists while a placed plant is selected (`game/hud.gd:919-924`, made visible at
  `:1347`), and nothing ever suggests selecting one. I searched for the BEHAVIOUR — "the game
  ...
  (only `:1434` "That upgrade costs N seeds", a refusal after you already tried, and `:1444`, a
  ...
  (`game/game.gd:279`) teaches placing and starting waves and stops there. So every mention of
...
- **591 kills produced 591 husks and not one was banked.** Every kill drops one
  (`game/game.gd:919`) and they live 4.5-10s on a value-scaled ramp (`HUSK_LIFETIME` and
...
- **A measurement driven by playing waves measures AMBIENT behaviour, and cycle 96's answer
  ...
  uproot (`game/game.gd:1915`) and opening a seed packet (`:2257`, `:2271`) are the only
  ...
  **ANSWERED IN CYCLE 128 (`-gd27`), and cycle 93's conclusion is CORRECTED: the row does
  ...
  (`MESSAGE_QUEUE_MAX`, `game/hud.gd:827`) and refuses two.
...
- **Arming an uproot destroys the line the player is reading, and that is the real cost of
  ...
  ones are the armed uproot (`game/game.gd:2052`, `MESSAGE_DEADLINE`) and the packet
  ...
  (`game/game.gd:1510-1512`: the countdown is already running, so deferring eats the window
  ...
  `MESSAGE_NORMAL` (`game/game.gd:1313`), so a player who arms an uproot in the same second
...
- **Nineteen of the game's twenty-two message-row calls sit on the SAME rung, and a tie
  ...
  (the packet flourish and its reveal) and one `MESSAGE_DEADLINE` (`game/game.gd:2052`,
...
- **The game has exactly ONE one-shot hint, and now has the machinery for a second.**
  ...
  **Enumerated rather than assumed**: `has_milestone` used as a SHOW gate appears once in the
  whole game, at `game/game.gd:1382`. Its other call sites read earned state to RENDER rather than to gate a
...
- **The move tip is now a courtesy that an upgraded plant never sees.** It fires on the
  first arm ever (`game/game.gd:1352-1354` records `HINT_MOVE_PREVIEW`), and from cycle 79
  ...
  milestone only when the tip is actually shown (one line, `game/game.gd:1353`), or accept
...
- **The options panel is exactly full, and the next audio idea needs a row.**
  ...
  gap is 48. `OverlayScreen.FOOTER_GAP` is 24 (`game/overlay_screen.gd:126`). **A fourth row
...
- **The only line with a clock behind it is the only clock the game never draws.**
  `_uproot_left` ticks down at `game/game.gd:1402` and lives entirely inside `game.gd` —
  outside it the identifier appears only in tests, never in `hud.gd`, so the HUD learns
  *armed or not* through `uproot_armed()` (`game/game.gd:2007`) and never *how much is
...
- **"Only one line may carry a deadline" is a paragraph, and paragraphs do not fail.**
  ...
  whichever waits is wrong by construction — and `game/game.gd:1360` is, today, the only
...
- **Three budgets sit exactly at their declared floor, and nothing on screen says the HUD
  ...
  1112 — each precisely its floor in `Game.BUDGET_FLOOR` (`game/game.gd:3024-3077`),
...
- **An evidence string can read two ways and only one of them is a working budget.**
  ...
  slot, and I misread it before opening `game/game.gd:2572`. Corrected. The general shape
...
- **`_ever_engaged` knows whether the garden ever touched a pest, and nothing draws it.**
  `game/pest.gd:1909-1910` is now its ONE writer — `_mark_engaged()`, whose own header at
  `:1902` says so — called from `:1624` on damage above zero, from `:1282` when a Chomp's
  mouth holds one, and from `:1309`. (This bullet used to cite two line numbers for a
  two-writer design that no longer exists; cycle 139 found the pair landing on an unrelated
  `var` declaration and on a blank line. The numbers are deliberately not repeated here —
  writing them down as prose makes them live citations again. The feature claim below is
  unchanged and still true; only the citations were dead.) `was_engaged()` reads it at
  `:1896`,
...
- **CORRECTED (cycle 66): an escape is not unmarked, and this entry said it was.**
  ...
  three" — is false. An escape plays `Sfx.PEST_ESCAPED` (`game/game.gd:937`), tints the
  exit cell on the lane-pressure map via `_note_lane_loss(..., true)` (`:1055`), and punches
...
- **Teaching a one-time lesson in a recurring message is a permanent tax.** The armed
  ...
  (`game/game.gd:2052`) declares 40 px for that row and 121 remain, so it passes and the
...
- **`_update_preview` now decides two things and is named for one.** It resolves the hover
  cell AND, since this cycle, which plant the hover is about (`game/game.gd:1723`). That
...
- **The selection panel has no room left and nothing prices it.** `_selection_label`
  ...
  `hud_selection_panel` entry in `Game.budget_entries()` (`game/game.gd:3075`) beside the
...
- **Six `show_message()` durations are hand-picked and nothing relates them.**
  ...
  cancelled, husk swept), 2.5s (mute, colourblind) — at `game/game.gd:270`, `:421`,
  `:1213`, `:1336`, `:1584`, `:1612`, `:1600`, `:1732`. **SUPERSEDED BY `-uhno`, CYCLE 136,
...
- **The waiver reasons are the best documentation of the message system and live in six
  scattered comments.** `game/game.gd:317`, `:1827`, `:2257`, `:2422`, `:2426`, `:2673` each
  carry
...
- **`show_message()` has TWENTY call sites and no single place says so.** (Was "eight"
  until cycle 140, when a `--against` pass caught all four of its citations pointing at a
  much older shape of `game.gd`. The count is now read from the tool that counts them:
  `message_corpus_check` prints `20 show_message() call site(s), 6 waived` on every run, so
  the number above is derived rather than recounted by hand — which is the whole complaint
  this entry is making, one level up.)
  `game/game.gd:317` (purchase refusal), `:2422` and `:2426` (mute), `:2673` (placement
  refusal), plus the `Hud.*_message` producers. The message-row budget had to
...
- **The prep note is measured at a wave number the game cannot reach.**
  **SUPERSEDED, re-read in cycle 131: `next_wave_note` no longer appears in `game/game.gd`
  at all.** `_budget_hud_message_row` (`game/game.gd:3399`) now measures one declared corpus
...
- **The pause card is 440px wide and its two widest rows are 384px.** `CARD_WIDTH`
  (`game/pause_screen.gd:89`) is now sized for the worst key name a player can bind,
...
- ~~**`Game.key_help()` returns every action, including the notebook's pager.**~~
  **Wrong — checked while writing the citation, which is the point of the rule that
  ...
  (`game/game.gd:56`), so the scope filter this entry proposed has been there all
...
- **The economy plant is the one plant with nothing on the board to read.** The cob
  ...
  only readout is `hud.gd:579`'s "Next %d seeds in %.0fs", which costs a click on that
...
- **An entry points at where the problem was, not where the fix landed.** "Starting a wave
  ...
  went into the handler (`game/game.gd:314-315`). Checking only the cited line would have
...
- **Drought does nothing at all to a wall, and that is a free seam for the weather
  ...
  `Plant.MAX_HEALTH * heal_fraction_for(next)` loop (`game/game.gd:435`).
...
- **Nothing in this project can tell whether a sentence is TRUE, and cycle 112 found one
  that had quietly stopped being.** `Hud.eaten_message` (`game/hud.gd:3929`) read "A hungry
  ...
  branch (`game/pest.gd:1312-1316`), so a hungry pest really was the only thing that could
...
- **One function returns both success sentinels, and a test comment already misreads it.**
  ...
  (`game/game.gd:1625` `place_plant`, `:1840` `upgrade_selected`, `:2090` `commit_uproot`).
  `arm_uproot` (`game/game.gd:1891`) breaks it in the most confusing available way: the
  first press returns `"confirm needed"` (`:1982`) and the second returns `""` — via
  `commit_uproot()` at `:2090` — so the SAME function uses the refusal sentinel for one
...
- **An armed move still draws its ring over ground the plant could never occupy.**
  ...
  `_preview.plant_id` from `previewing` unconditionally (`game/game.gd:2523-2527`). So the
...
- **A sentence that interpolates what it describes cannot outlive it; one that names a
  ...
  `eaten_message`'s "A hungry pest" (fixed cycle 112, `game/hud.gd:3929`) and
...
- **The notebook is structurally derived nearly everywhere, and both places it is not were
  ...
  five here" beside a six-row table (`game/cue_legend.gd:191`). **The page displayed a
...
- **A five-lane merge drifted 105 citations, and a blanket per-file offset would have
  ...
  `game/game.gd:1904` now records the rename. Marked superseded with the derived answer
