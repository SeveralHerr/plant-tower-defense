...
## Cycle 181 — a 4-lane fan-out, then the file-size question it left open

First half: a 4-lane parallel fan-out (`b0mp`, `h3jz`, `mvsa`, `vlpg`) plus two beads
resolved directly (`owl6` was already reconciled at cycle 37/152 despite reading open;
`u3zp` got its tell added to `.claude/skills/house-static-checker/SKILL.md`). All four
lanes merged with zero conflicts except one auto-merged hunk in `test_placement.gd`;
1069/1069 tests green on the merged tree. Full detail is in the six beads' own close
reasons — this file only carries what `bd` can't.

**THE DECISION `plant-tower-defense-k5y5` ASKED FOR.** Thirty files are over 500 lines;
the four biggest are `test/unit/*.gd` (48762 of ~76000 surveyed lines), and CLAUDE.md's
harness section says explicitly not to split `test_selftest.gd` — a check written there
is inherited by every later `/verify` run, which splitting would break. Two rules were in
tension and the bead asked which wins.

**Decision: CLAUDE.md wins for `test/unit/`.** That block is generated by
`/scaffold-godot-harness` from the upstream `godot-selftest-harness` project — changing
the rule here would be overwritten on the next refresh, so it isn't this repo's call to
make unilaterally. The 500-line convention applies to `game/` and `tools/` only, which
narrows the survey to 23 files (re-measured post-fan-out; game.gd grew to 4544 lines from
b0mp's extraction, placement_preview.gd crossed 500 from vlpg's conversion). The two that
matter are unchanged: `game/game.gd` (4544) and `game/hud.gd` (4405) — a large drop to
`game/pest.gd` (2026) after them.

Filed as their own beads rather than done here, because each is a real undertaking with
its own sequencing constraint, not a fan-out-able afternoon: `plant-tower-defense-2dlh`
(game.gd — sequenced after `test_mirror_parity.gd` was proven to still catch a mutation
post-b0mp, which it was) and `plant-tower-defense-tar5` (hud.gd — sequenced after the
open beads that cite its line numbers, or budget a `citation_rebind.py` pass alongside).
Neither was attempted this cycle; splitting either is a bigger, riskier undertaking than
belongs in the same pass as a 4-lane fan-out, and `game.gd` in particular sits right next
to the parity gate this cycle just finished trusting.
