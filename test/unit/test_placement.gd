...
# ---------------------------------------------------------------------------
# THE REACH RING, once (plant-tower-defense-snnp)
#
...
# `Plant.draw_reach_ring()` is now the only implementation, and these two tests are
# the pair that keeps it the only one — one for the NUMBER, one for the WIRING,
# because a base-class draw helper can be perfectly correct and still never run.
# ---------------------------------------------------------------------------
...
## `reach_ring_radius()` can return the right number, the test above can pass, and the
## ring still never appears on the two plants most likely to want one.
##
...
func test_every_plant_that_paints_more_than_its_reach_still_calls_the_ring() -> String:
	...
	for id: StringName in PlantCatalog.ids():
		...
		if path == "":
			...
			break
		var code: String = _code_without_comments(FileAccess.get_file_as_string(path))
		if code == "":
...
## GDScript source with every whole-line comment removed, so a name mentioned in a doc
## block does not count as a call. Deliberately crude — it does not strip trailing
## comments or string literals, because neither appears in the question being asked and
## a half-written parser is worse than a rule a reader can hold in their head.
