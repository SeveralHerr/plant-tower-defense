...
## Every tier's tooltip counts what that tier can actually reach.
##
## Written against SeedBank.PACKET_ORDER rather than against `common` and `rare`,
## because naming a tier is the same perishable mistake naming a plant was. The
## previous version asserted that the RARE tooltip counted the whole catalogue —
## true while rare was the top tier, false the moment `epic` landed above it and
## rare's cap dropped to 2, and it failed as a broken test rather than as a broken
## tooltip. The claim that survives is positional: the LAST tier reaches
## everything, and every tier before it says how much it cannot.
func test_a_packet_tooltip_counts_what_its_tier_can_actually_reach() -> String:
	...
	var total: int = PlantCatalog.ids().size()
	var err: String = _T.assert_gt(order.size(), 1,
		"there is more than one tier, or a cap means nothing")
	if err != "":
		return err
	for index: int in order.size():
		var tier: StringName = order[index]
		var text: String = Hud.packet_tooltip(tier)
		...
		for id: StringName in PlantCatalog.ids():
			if PlantCatalog.tier(id) <= cap:
				...
				beyond += 1
		err = _T.assert_gt(within, 0, "%s can reach something at all" % tier)
		if err == "":
			err = _T.assert_true(text.contains(str(within)),
				"the %s tooltip counts what it can reach (%d), got: %s" % [tier, within, text])
		if err == "":
			if index < order.size() - 1:
				err = _T.assert_gt(beyond, 0,
					"%s is not the top tier, so something has to be out of its reach" % tier)
				if err == "":
					err = _T.assert_true(text.contains(str(beyond)),
						"and it says how many it cannot (%d), got: %s" % [beyond, text])
			else:
				err = _T.assert_eq(beyond, 0,
					"the top tier (%s) reaches every plant, %d left over" % [tier, beyond])
				if err == "":
					err = _T.assert_true(text.contains(str(total)),
						"and its tooltip counts the whole catalogue (%d), got: %s" % [total, text])
		if err != "":
			return err
	return _T.assert_true(Hud.packet_tooltip(&"nosuchtier").is_empty(),
...
# -- The Bomb Dandelion (plant-tower-defense-2gd) ------------------------
#
# Three things were asked for together and only make sense together: a fifth
# plant whose seeds ARC and DETONATE rather than travelling straight, a head that
# visibly loses fluff as it fires, and an `epic` packet tier above common and
# rare that is the only way to reach it. The checks below are grouped the same
# way.
