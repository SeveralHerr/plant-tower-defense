...
NOT_A_CHECKER = {
    ...
    "gen_bool_classifier.py": "a GENERATOR, not a check, and the second of its shape after "
                              "gen_pulse_cue.py above: it derives a GDScript bool-input "
                              "if-chain AND its test block from one JSON spec so the two "
                              "cannot drift. It arrived (806a4de) with NO contract line at "
                              "all and was reported UNCLASSIFIED, which is that gate doing "
                              "exactly its job -- a new tools/*.py that is neither derived "
                              "nor listed fails the run rather than silently becoming "
                              "nothing. Its real gate is `--check-func`/`--check-test`, "
                              "which honour the exit-code contract but require a spec path, "
                              "and a tool that cannot run bare can never be a checker here. "
                              "Its docstring states its blind spot WITHOUT the contract "
                              "marker, on purpose -- an entry here plus the marker is "
                              "UNCLASSIFIED, not doubly-safe, which this file said in so "
                              "many words when the first draft tried exactly that",
