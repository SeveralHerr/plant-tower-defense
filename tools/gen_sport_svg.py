...
A bare run is a house checker in the sense `.claude/skills/house-static-checker` means
it -- writing is behind `--write` for the reason `main` gives -- so it owes that
paragraph. The defect class is "a committed `_sport.svg` no longer matches
...
## The banner every generated file carries. No double hyphen anywhere in it, and that is
## a hard constraint rather than a style choice: XML forbids `--` inside a comment, so
## naming a double-dash flag here would make every sport SVG malformed. Godot's own SVG
## loader accepts it happily and rendered all seventeen; `svg_style_check.py` parses with
## ElementTree and refused all seventeen, which is the whole argument for having a second
## reader of the same file.
BANNER = (
    "  <!-- GENERATED by tools/gen_sport_svg.py from %s. Do not edit by hand: run\n"
    "       `python tools/gen_sport_svg.py` and it will report any edit this file's\n"
...
## drawing and nothing may collapse -- a bare run fails loudly rather than flattening a
## sprite that outgrows the ramp, which is the failure a tenth plant would hit.
##
## The palest anchor of each ramp stops well short of white, and that is a constraint
## rather than taste: `svg_style_check.check_outline` warns when a saturated stroke rims
## a GREY fill, and it decides grey at saturation 0.12. The mutagen ramp's top anchor was
## (255, 232) on its first draft -- saturation 0.090 -- which made the Aloe's leaves read
## as a coloured rim around bare paper and drew that warning on five shapes at once. Both
## top anchors now sit near 0.2. Anything paler is a highlight the outline contract does
## not believe is a colour.
RAMPS = {
...
MUTANT_PALETTE_RE = re.compile(r"const\s+MUTANT_PALETTE[^\[]*\[(.*?)\]", re.S)


def gate_mutant_palette(root):
    """The MUTANT_PALETTE entries `test_sprite_style.gd` currently carries, in order.
    ...
    if not os.path.isfile(path):
        return None
    m = MUTANT_PALETTE_RE.search(read_text(path))
    if not m:
        return None
    return [h.upper() for h in re.findall(r'"([0-9a-fA-F]{6})"', m.group(1))]


def ramp_hexes():
    """Every ramp anchor, in the order `palette_block` prints them."""
    return [to_hex(c) for fam in (TOXIC, MUTAGEN) for c in RAMPS[fam]]
...
def first_difference(got, want):
    for i in range(max(len(got), len(want))):
        ...
        if a != b:
            return "index %d: gate #%s vs ramp #%s" % (i, a, b)
    return "(none)"
...
def main(argv=None):
    ...
    # Checking is the DEFAULT and writing is the flag, which is backwards for a
    # generator and deliberate for this one: `check_all.py` discovers every tools/*.py
    # carrying a NOT COVERED line and runs it bare, in parallel, on whatever tree the
    # agent happens to have. A generator that wrote by default would silently rewrite
    # seventeen committed files as a side effect of running the checkers.
    ap.add_argument("--write", action="store_true",
                    help="regenerate the sport SVGs (default: report drift, write nothing)")
    ...
    gate = gate_mutant_palette(root)
    if gate is None:
        print("gen_sport_svg: could not run -- no MUTANT_PALETTE block in %s. That block\n"
    ...
    for stem in stems:
        ...
        if not args.write:
            ...
            findings.append(
                "FINDING: %s/%s %s\n"
                "  fix: run `python tools/gen_sport_svg.py --write`, then re-render with\n"
    ...
    if gate != ramp_hexes():
        findings.append(
            "FINDING: MUTANT_PALETTE in %s is not this file's RAMPS -- %d gate entr(y/ies)\n"
            "         against %d ramp anchor(s), %s\n"
            "  fix: `python tools/gen_sport_svg.py --palette`, paste the block over it.\n"
            "  waive: none -- a sport painted in a colour the gate does not carry fails\n"
            "         test_every_colour_is_kit_palette_or_a_blend_of_two, and an entry the\n"
            "         gate carries that no ramp emits silently widens what a sport may use."
            % (GATE_SCRIPT, len(gate), len(ramp_hexes()),
               first_difference(gate, ramp_hexes())))
